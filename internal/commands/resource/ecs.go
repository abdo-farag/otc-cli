...
func ListECS(cfg *config.Config, client *otc.Client, unscopedToken, projectID string, options map[string]interface{}, raw bool) {
	...
	// Use centralized ECS endpoint
	computeURL := fmt.Sprintf("%s/v1/%s/cloudservers/detail", cfg.Endpoints.ECS, projectID)
...
func GetECS(cfg *config.Config, client *otc.Client, unscopedToken, projectID, serverNameOrID string, raw bool) {
	...
	// First, try to find the server by listing all servers
	computeURL := fmt.Sprintf("%s/v1/%s/cloudservers/detail", cfg.Endpoints.ECS, projectID)
	...
	var targetServer *cloudservers.CloudServer
	...
	// Now get the detailed server information using the ID
	authOpts := golangsdk.AuthOptions{
		IdentityEndpoint: cfg.Endpoints.IAM,
	...
	// Use SDK's Get method with the actual server ID
	server, err := cloudservers.Get(ecsClient, targetServer.ID).Extract()
	if err != nil {
		color.Red("✗ Failed to get server details: %v", err)
		return
	}
